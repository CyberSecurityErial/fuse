...
fuse::GemmA2AParams make_params(
  ...
  params.num_comm_ctas = resolved_comm_ctas;
  params.epoch = 1;
  return params;
}
...
int run(const Options& options, RankContext& context) {
  ...
  auto params = make_params(
      options, context, buffers, n, resolved_comm_ctas);
  ...
  if (options.check) {
    ...
    validate_numerics(context, runtime, buffers, params);
  }
  if (context.rank == 0) {
