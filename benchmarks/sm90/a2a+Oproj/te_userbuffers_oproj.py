...
def run(
    ...
    if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        ...
        record_inputs(source, weight, seed=2701+rank, label="oproj_ub")
    weight_shards = [
    ...
    # GEMM algorithm selection occurs before the first A2A. The receive slab
    # has no valid payload yet: seed it with actual random activation data.
    # This initialization is outside all boundary timing and graph capture.
    recv[0].copy_(source.reshape(-1, k_local)[:m])
    if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        record_inputs(recv[0], weight_shards[0], seed=2701+rank, label="oproj_ub_gemm_tuning")
    ...
    if args.check and os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        from measurement import oproj_ub_check
        boundary()
        torch.cuda.synchronize(device)
        check, expected = oproj_ub_check(source, weight, send, recv, output, rank, world)
    elif args.check:
        boundary()
        torch.cuda.synchronize(device)
    ...
    if args.check and args.cuda_graph:
        ...
        if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
            ...
            post_graph_max_abs = max_abs_difference(output, expected)
        else:
