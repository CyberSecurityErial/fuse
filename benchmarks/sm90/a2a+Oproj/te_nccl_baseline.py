...
def run_oproj(
    ...
    if args.metric_profile != "route":
        ...
        if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
            ...
            record_inputs(attention_output, linear.weight, seed=2701+rank, label="oproj_nccl")
    ...
    check: dict[str, int] = {}
    if args.check and os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        from measurement import inverse_expected, max_abs_difference, count_mismatches
        expected = inverse_expected(attention_output.reshape(args.global_seq, -1), rank, world, te_order=True)
        mismatch = count_mismatches(staging, expected)
        ...
        if args.metric_profile != "route":
            ...
            max_abs = max_abs_difference(cublas_output, reference_output)
            del reference_output
            dist.all_reduce(max_abs, op=dist.ReduceOp.MAX)
            check["cublaslt_vs_torch_mm_max_abs"] = float(max_abs.item())
            if not torch.isfinite(max_abs) or max_abs.item() > .01:
                raise RuntimeError(f"cuBLASLt numerical error: {check}")
    elif args.check:
