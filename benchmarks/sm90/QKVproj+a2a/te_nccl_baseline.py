...
class CublasLtRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self.info.update(m=m, n=n, k=k, sm_count_target=sm_count_target)
        if hasattr(self.library, 'fuse_cublaslt_bf16_cache_hit'):
            self.library.fuse_cublaslt_bf16_cache_hit.argtypes = [ctypes.c_void_p]
            self.library.fuse_cublaslt_bf16_cache_hit.restype = ctypes.c_int
            self.info['disk_cache_hit'] = bool(self.library.fuse_cublaslt_bf16_cache_hit(self.handle))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cublaslt-library",
        type=Path,
        default=Path(__file__).resolve().parents[3] / "build" / "libfuse_cublaslt_runner.so",
    )
...
def timed_critical(
    ...
    *,
    ...
    if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        from measurement import timed_critical as measured
        return measured(fn, warmup, iterations, device, use_cuda_graph=use_cuda_graph)
...
class UlyssesRoutes:
    ...
    def __init__(
        ...
        *,
        ...
        # The SM103 full-QKV boundary never calls the optional QK/V-only routes.
        # Allocate zero-sized placeholders for them instead of multi-GiB buffers.
        aux_seq = 0 if full_only else seq_local
        qk_local_heads = self.local_q_heads + self.local_kv_heads
        qk_shape = (world, batch, aux_seq, qk_local_heads, head_dim)
        ...
        self.qk_output = torch.empty(
            (batch, world, aux_seq, qk_local_heads, head_dim),
            dtype=torch.bfloat16,
            device=device,
        )

        v_shape = (world, batch, aux_seq, self.local_kv_heads, head_dim)
        ...
        self.v_kv = torch.empty(
            (batch, self.local_kv_heads, world, aux_seq, head_dim),
            dtype=torch.bfloat16,
            device=device,
        )
...
def exact_full_qkv_route_check(
    ...
    *,
    ...
    if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        from measurement import qkv_route_check
        assert batch == 1
        return qkv_route_check(qkv, actual, seq_local, q_heads*head_dim, kv_heads*head_dim, rank, world)
    peers = [torch.empty_like(qkv) for _ in range(world)]
    dist.all_gather(peers, qkv)
    q_local, kv_local = q_heads // world, kv_heads // world
    q_begin, kv_begin = rank * q_local, rank * kv_local
...
def run(args: argparse.Namespace, rank: int, world: int, device: torch.device):
    ...
    seed = (3109 if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2" else 1701) + rank
    ...
    if args.metric_profile != "route":
        ...
        if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
            with torch.no_grad():
                linear.weight.uniform_(-0.02, 0.02, generator=generator)
        dist.broadcast(linear.weight, src=0)
        if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
            from measurement import record_inputs
            record_inputs(x, linear.weight, seed=seed, label="qkv_nccl")
        cublaslt = cached_cublaslt_runner(
    ...
    routes = UlyssesRoutes(
        ...
        full_only=os.environ.get("FUSE_SM103_MEASUREMENT") == "v2" and not args.include_qk,
    )
    cp_stream = torch.cuda.Stream(device=device)
    chunk_ids = get_seq_chunk_ids_for_reordering_before_attn(world, device)
    ...
    if args.check and args.metric_profile != "route" and os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        from measurement import max_abs_difference
        reference_projection = torch.mm(x, linear.weight.t())
        cublaslt_project()
        error = max_abs_difference(qkv_cublas, reference_projection)
        del reference_projection
        dist.all_reduce(error, op=dist.ReduceOp.MAX)
        check["cublaslt_vs_torch_mm_max_abs"] = float(error.item())
        if not torch.isfinite(error) or error.item() > .01:
            raise RuntimeError(f"cuBLASLt QKV numerical error: {check}")
    ...
    def measure(
        ...
        *,
        ...
        # SM103's tuned NCCL baseline requests only the cuBLASLt boundary.
        # Historical SM90/full-profile comparisons keep their original metrics.
        if (os.environ.get("FUSE_SM103_MEASUREMENT") == "v2"
                and args.metric_profile == "boundary" and not args.include_te
                and name != "cublaslt_packed_qkv_gemm_a2a"):
            return
        samples = timed_critical(
            fn,
            args.warmup,
