...
def run(
    ...
    if os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        ...
        record_inputs(source, weight, seed=3109+rank, label="qkv_ub")
    ...
    if args.check and os.environ.get("FUSE_SM103_MEASUREMENT") == "v2":
        from measurement import qkv_ub_check
        boundary()
        torch.cuda.synchronize(device)
        check, expected = qkv_ub_check(source, weight, send, recv, output, rank, world, q_width, kv_width)
    elif args.check:
        boundary()
        torch.cuda.synchronize(device)
