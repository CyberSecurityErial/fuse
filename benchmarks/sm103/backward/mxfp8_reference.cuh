...
struct Workspace {
  ...
  fused_validation::Stats validate(Operand a, Operand bt, const fuse::Bf16* actual,
                                  ...
                                  fuse::Bf16* cache=nullptr, bool reuse_cache=false) {
    return validate_views(a,bt,actual,m,n,k,stream,cache,reuse_cache);
  }
  template<class Left,class Right>
  fused_validation::Stats validate_views(Left a, Right bt, const fuse::Bf16* actual,
                                        int m,int n,int k,cudaStream_t stream,
                                        fuse::Bf16* cache=nullptr, bool reuse_cache=false) {
    if (!handle || m <= 0 || n != columns || k <= 0 || k % 32 || (reuse_cache && !cache))
    ...
    for (int row = 0; row < m; row += kRows) {
      ...
      auto* expected_slab = cache ? cache + int64_t(row)*n : expected;
      // Optional storage is owned by the caller, valid only while the ORIGINAL
      // operands and geometry are unchanged. Cache independently computed
      // reference values, never actual output. Every invocation still compares
      // every element with the original checker/tolerance. Default callers keep
      // the bounded-memory path and recompute each slab as before.
      if (!reuse_cache) {
        ...
        finish<<<256,256,0,stream>>>(accumulator,expected_slab,uint64_t(rows)*n);
        check(cudaGetLastError());
      }
      check(fused_validation::launch<true>(reinterpret_cast<const uint16_t*>(actual+int64_t(row)*n),
          fused_validation::DenseOracle{reinterpret_cast<const uint16_t*>(expected_slab)},
