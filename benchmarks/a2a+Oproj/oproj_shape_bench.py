...
def comparison_table(args: argparse.Namespace) -> None:
    calibrated = args.results / "fused_calibrated_summary.json"
    if not calibrated.exists():
        raise FileNotFoundError(
            "missing fused_calibrated_summary.json; run "
            "--phase fuse-calibrated-aggregate first"
        )
    fused = load(calibrated)
