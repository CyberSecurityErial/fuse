...
// BF16 reverse routes mirror SM90's semantic contract; SM103 uses the shared
// Blackwell persistent dispatcher. Ready covers a full (M tile, head), after
// both 64-row transfers complete. The remote producer publishes at system
// scope; the receiving GEMM acquires at system scope before async-proxy TMA.
namespace fuse {
namespace {

inline bool supported_route_base(const UlyssesRoute& r) {
  return r.world_size > 0 && r.world_size <= kMaxWorldSize && r.rank >= 0 &&
      r.rank < r.world_size && r.batch > 0 && r.seq_local > 0 && r.global_seq > 0;
}
...
struct QkvBackwardPushCommT {
  ...
  static constexpr int kQkvBulkSlots = 8;
  static constexpr int kQkvBulkRows = 64;
  static constexpr int kQkvBulkColumns = 128;
  static constexpr int kQkvBulkStageBytes = kQkvBulkRows * kQkvBulkColumns * sizeof(Bf16);
  static constexpr int kMinThreads = kQkvBulkSlots * 32;
  ...
  static bool can_implement(const Arguments& args) {
    const auto& p = args.params;
    const auto& route = p.route;
    if (!supported_route_base(route)) return false;
