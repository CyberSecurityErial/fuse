...
template <class Gemm>
detail::A2AInputTileOrder a2a_input_order(const typename Gemm::Arguments& args) {
  ...
  const int32_t m_tiles = ceil_div(
      static_cast<int32_t>(cute::get<0>(args.problem_shape)),
      static_cast<int32_t>(cute::size<0>(typename Gemm::TileShape{})));
  return detail::A2AInputTileOrder::make(schedule, m_tiles);
}
...
cudaError_t launch_a2a_lhs_gemm_policy(
  ...
  comm.input_order = a2a_input_order<Gemm>(args);
