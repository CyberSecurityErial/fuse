...
`csrc/operators/sm90/entry.cu` is only the assembly point. The implementation
...
- `detail/gemm.cuh`: scalar helpers, kernel geometry, calibrated policy data,
...
- `detail/persistent_gemm.cuh` and `cutlass_pipeline.cuh`: Hopper-only
  scheduler and collective adapters, moved out of the shared include tree.
