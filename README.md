...
QKV 前向典型泛化场景 **2.228 PFLOPS/卡**（27点），比 v24.0 历史结果 +1.24%；
包含补充 QwenDense 的全33点为2.077P（+1.47%）。保留完整计时边界与双输入10+50。
优化 all 模式的发布与寻址；注明融合量化的并行配置限制，并保留显式独立量化对照。
OProj 和两个反向沿用 v24.0；不纳入无整体收益的试验。
[完整表、配置和已知 profiling 限制](results/sm103/v24.1/README.md)。
...
| QKVProj（各27点） | 2.201 | 2.081 |

QwenDense 作为补充场景保留全部逐点结果，不计入主汇总；包含它的 QKV 原全量均值仍为前向 2.047P、反向 1.870P（各33点）。这是发布后统计分组调整，不是性能提升或普遍泛化证明。
...
固定配置 Graph10+50 几何平均 **2.256 PFLOPS/卡**，相较 v21 历史结果
**+17.85%**，达到历史满 SM cuBLASLt 的 **83.1%**；不是每点均超过2.2P。

计算 CTA 在启动阶段协助权重量化，通信立即搬运激活；均衡 scale 交货，
按实际数据量安排搬运槽，并让通信窗口/预算适配 GEMM 消费顺序。
采用显式离线配置，不宣称新的 Auto 已完成。上游激活量化不计入本边界。

保存当前开发进展：OProj MXFP8 完整 B+dW 已有经校验的初始基线，尚未达到2P；
QKV 前向继续优化，QKV MXFP8 反向尚未实现。norm/RoPE 仍是默认关闭的实验功能。
[最终全表、配置、复现与限制](results/sm103/v23.0/README.md)。
