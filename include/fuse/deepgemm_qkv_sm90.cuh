...
// Derived from DeepGEMM sm90_bf16_gemm.cuh (DeepSeek, MIT License).
#pragma once
...
// Normal-GEMM subset of DeepGEMM's persistent scheduler with a physical CTA
// prefix reserved for communication. The swizzle and multicast pairing remain
// identical to the upstream scheduler; only the logical block id is rebased.
template <
    uint32_t BLOCK_M,
    uint32_t BLOCK_N,
    uint32_t kNumMulticast,
    bool kIsMulticastOnA,
    uint32_t kNumComputeCtas>
struct OffsetScheduler {
    int current_iter = -1;
    uint32_t num_blocks;
    ...
    uint32_t logical_cta;
    bool is_peer_cta_alive = true;

    static constexpr uint32_t group_size() {
        constexpr uint32_t usage8 = kIsMulticastOnA
            ? 8 * BLOCK_N + (kNumComputeCtas + 7) / 8 * BLOCK_M
            : 8 * BLOCK_M + (kNumComputeCtas + 7) / 8 * BLOCK_N;
        constexpr uint32_t usage16 = kIsMulticastOnA
            ? 16 * BLOCK_N + (kNumComputeCtas + 15) / 16 * BLOCK_M
            : 16 * BLOCK_M + (kNumComputeCtas + 15) / 16 * BLOCK_N;
        return usage8 < usage16 ? 8 : 16;
    }

    CUTLASS_DEVICE OffsetScheduler(
        uint32_t shape_m,
        uint32_t shape_n,
        uint32_t shape_k,
        ...
        : num_m_blocks(math::ceil_div(shape_m, BLOCK_M)),
          ...
          logical_cta(static_cast<uint32_t>(blockIdx.x) - block_offset) {
    ...
    CUTLASS_DEVICE void get_swizzled_block_idx(
        ...
        constexpr uint32_t group = group_size();
        static_assert(group % kNumMulticast == 0);
        const uint32_t primary_blocks =
            kIsMulticastOnA ? num_n_blocks : num_m_blocks;
        const uint32_t secondary_blocks =
            kIsMulticastOnA ? num_m_blocks : num_n_blocks;
        const uint32_t blocks_per_group = secondary_blocks * group;
        const uint32_t group_idx = block_idx / blocks_per_group;
        uint32_t first_block = group_idx * group;
        uint32_t in_group = block_idx % blocks_per_group;
        num_blocks_in_group = min(group, primary_blocks - first_block);
        if (kNumMulticast > 1 && num_blocks_in_group % 2 != 0) {
            const uint32_t paired_blocks = num_blocks_in_group ^ 1;
            if (in_group < paired_blocks * secondary_blocks) {
                num_blocks_in_group = paired_blocks;
            } else {
                in_group -= paired_blocks * secondary_blocks;
                first_block += paired_blocks;
                num_blocks_in_group = 1;
            }
        }
        if constexpr (kIsMulticastOnA) {
            m_block_idx = in_group / num_blocks_in_group;
            n_block_idx = first_block + in_group % num_blocks_in_group;
        } else {
            m_block_idx = first_block + in_group % num_blocks_in_group;
            n_block_idx = in_group / num_blocks_in_group;
        }
    }

    template <bool, IndexType = IndexType::MN>
    ...
    CUTLASS_DEVICE bool get_next_block(
        ...
        const uint32_t block_idx =
            static_cast<uint32_t>(++current_iter) * kNumComputeCtas +
            logical_cta;
        ...
        get_swizzled_block_idx(block_idx, m_block_idx, n_block_idx);
        return true;
    }

    CUTLASS_DEVICE bool is_tma_multicast_valid(uint32_t) const {
        return true;
    }
};
...
sm90_bf16_gemm_fused_impl(
                    CUTLASS_GRID_CONSTANT typename CommOp::Params const comm_params,
                    uint32_t num_comm_ctas,
                    bool enable_comm,
                    uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    if (static_cast<uint32_t>(blockIdx.x) < num_comm_ctas) {
        if (enable_comm) {
            CommOp{}(
                comm_params,
                static_cast<int32_t>(blockIdx.x),
                static_cast<int32_t>(num_comm_ctas));
            cooperative_groups::this_grid().sync();
            CommOp{}.finalize(comm_params);
        }
        return;
    }
    ...
    DG_STATIC_ASSERT(kGemmType == GemmType::Normal, "Fused path supports normal GEMM");
    DG_STATIC_ASSERT(BLOCK_M == 128 and kNumMathThreads == 256,
                     "Fused signaling assumes two complete math warp groups");
    ...
    auto scheduler = sched::OffsetScheduler<
        BLOCK_M,
        BLOCK_N,
        kNumTMAMulticast,
        kIsTMAMulticastOnA,
        kNumSMs>(shape_m, shape_n, shape_k, num_comm_ctas);
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        constexpr uint32_t WAVE_BLOCK_M =
            BLOCK_M <= WGMMA::M ? BLOCK_M : WGMMA::M * 2;
        constexpr uint32_t kNumElemBytes = sizeof(nv_bfloat16);
        constexpr uint32_t TMA_D_BLOCK_N =
            kSwizzleDMode == 0 ? BLOCK_N :
            (kSwizzleDMode / kNumElemBytes);
        constexpr uint32_t WGMMA_M_PER_WARP = WGMMA::M / 4;
        constexpr uint32_t kNumWGMMAStoreThreads =
            WAVE_BLOCK_M * (128 / WGMMA::M);

        const uint32_t output_n_tiles = math::ceil_div(shape_n, BLOCK_N);
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            if (threadIdx.x == 0) {
                const uint32_t signal =
                    m_block_idx * output_n_tiles + n_block_idx;
                fuse::detail::store_release_system(
                    comm_params.params.ready +
                        signal * fuse::kReadyFlagStride,
                    comm_params.params.epoch);
            }
        }
    }
    if (enable_comm) {
