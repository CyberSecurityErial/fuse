...
- 按连续物理 chunk 寻址；保留 K32 数值、完整 ready 和消费端异步内存序。
- 典型27点2.227942P（历史对照+1.24%）；含补充点的全33点2.077387P（+1.47%）。
- 独立量化可选择自己的并行配置；显式对照必须把量化与 GEMM+A2A 一起计时。
- OProj/反向沿用 v24.0；新版 all 详细量化发布 trace 暂不支持，显式拒绝而不伪造数据。
  [最终结果与复现](results/sm103/v24.1/README.md)。
...
- 已量化 activation，kernel 内动态 BF16 权重量化，MXFP8 GEMM/FP32 累加、
  BF16 输出 A2A；K32 数值契约与完整 panel-ready，持久化 CTA 分工。
- GEMM 参数是输入；`num_comm_ctas=0` 只做编译进代码的离线服务模型推导，
  不在线试跑，不拟合融合赢家；正值覆盖，不支持域明确拒绝。
- 33/33 物理点 Graph10+50、双 payload 校验。对同场手工最优配置 GM96.69%、
  最低90.09%，22个长序列 holdout GM97.33%。历史原值另列，Dense CP4/512K
  对历史88.39%，不误说所有历史分母也通过。保留独立手工最优配置和最终结果。
- SM90/BF16不扩性能承诺；MXFP8 OProj、反向与特殊路由不是本版交付内容。
  [最终结果](results/sm103/v20.0/README.md)。

## v18.0：SM103 OProj 运行时通信预算模型

- 运行时从8/16/24/32/48个通信CTA中预测选优，固定调用者的GEMM配置。
  独立C/R服务曲线与真实raster/swizzle、copy槽位、完整ready依赖共同评分；
  使用按物理请求索引的线程局部缓存，不保存模型名或逐shape优胜配置。
- 公共入口以`num_comm_ctas=0`启用，显式正预算不变；未标定请求明确不支持，
  不线性外推SM吞吐。默认N128仍需显式通信预算；联合GEMM/CTA自动plan为TODO。
- 五模型长序列30点中23点完成46次A/B，7个历史缺测留空。Graph 1+5快测
  Auto相对v17最终配置本轮重放几何平均−1.21%，相对满148SM历史纯Lt保留85.28%。
  9个换预算点为−3.14%；Qwen3 CP8/512K退化10.62%。12/46候选、7/23点有
  超5%漂移标记，保留原始数据，不通过逐shape回填隐藏回退。
- 相对最早可比归档（Llama v14、其他v16），20点提升、3点下降，几何平均+11.46%；
  跨版本/时段/采样合同的历史比较不冒充严格配对。SM90与设备kernel/同步协议不变。
  [表、配置、精简样本与证据](results/sm103/v18.0/README.md)。

## v17.0：SM103 OProj 调度与离线选优基线

- OProj 交货队列适配实际 tile/raster/swizzle、持久化计算 CTA 步长及通信槽位；
  多 N-band AlongM 使用容量受限的 M cohort，AlongN 保留分组对角调度。
  完整 `(M, peer)` ready 单元和发布/获取协议不变，不恢复细粒度 K 信号或 push。
- 保存独立 GEMM/COPY 标定、预算受限 GEMM 对照、可审计 profiling 与离线性能模型。
  模型以 C/R 实测服务及生产消费布局评分，不按模型名硬编码；运行期自动选优仍为 TODO。
- 五个大模型128K/256K/512K、CP4/8：23点通过完整数值/路由校验，7点留空。
  Graph 1+5快筛的模型Top2实测选优，20点提升、3点持平，几何平均+15.77%；
  相对历史满148SM纯cuBLASLt吞吐保留率85.83%。5点采样漂移>5%，不称稳定收益。
- SM90冻结；没有本轮QKV全量或低精度/反向性能承诺。历史正式bench原地保留。
  [表、配置、精简样本与证据](results/sm103/v17.0/README.md)，不打包大profile或临时依赖。
